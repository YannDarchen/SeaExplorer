function [wg,att_deg,U,att,Fg,Fb,Fl,Fd,vx] = flight_model(p,rho,pitch,dvbp,temp,Vg,eps,Cd0,mg)
%%%%%
...
%att = min(abs(att1),abs(att2)); % keep the smallest root 
att = sign(dvbp).*min(abs(att1),abs(att2)); % keep the smallest root 
att_deg = att*(180/pi);
% Vertical forces
Fg = mg*g;
